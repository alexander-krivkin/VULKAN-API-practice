...
namespace ak
{
	int Application::run()
	{
		try
		{
			...
			initLoggingSystem_(hDebugEdit_, state_.logLevel, "debug.log");
#endif // _DEBUG
			initVulkanSystem_();
			mainLoop_();
		}
	...
	void Application::postMessage_(const LogLevelFlagBits logLevel, const std::wstring& message)
	{
#ifdef _DEBUG
	...
	void Application::initWin32_()
	{
		...
		// Set the image list.
		SendMessage(hToolbar_, TB_SETIMAGELIST,
			(WPARAM)ImageListID,
			(LPARAM)g_hImageList);

		// Load the button images.
		SendMessage(hToolbar_, TB_LOADIMAGES,
		...
		// Add buttons.
		SendMessage(hToolbar_, TB_BUTTONSTRUCTSIZE, (WPARAM)sizeof(TBBUTTON), 0);
		SendMessage(hToolbar_, TB_ADDBUTTONS, (WPARAM)numButtons, (LPARAM)&tbButtons);
		...
		if (!(hState_ = CreateWindowEx(
			...
			state_.parentClientRect_.right - 2 * state_.stateMargin_ - 2 * state_.workspaceMargin_,	state_.stateHeight_,
		...
		if (!(hWorkspace_ = CreateWindowEx(
			...
			state_.workspaceMargin_, state_.workspaceMargin_ + state_.toolbarHeight_,
			state_.parentClientRect_.right - 2 * state_.workspaceMargin_,
...
#ifdef _DEBUG
	void Application::initLoggingSystem_(const HWND& hWnd, uint32_t logLevel, std::string logFilename)
	{
		shpLoggingSystem_->init(hWnd, logLevel, logFilename);
	...
	{
		MSG msg{};
		state_.tStartFPS_ = std::chrono::steady_clock::now();

		while (true)
		{
			state_.tStart_ = std::chrono::steady_clock::now();

			state_.hStateText_ = L"FPS: " + std::to_wstring(state_.FPS_) +
				L"  | Frame: " +
			...
			state_.hStateText_ += state_.keyboardMouseFlags_ & KeyboardMouseFlagBits::eKeyLeftShift ? L" LSHIFT " : L"        ";
			state_.hStateText_ += state_.keyboardMouseFlags_ & KeyboardMouseFlagBits::eKeyRightShift ? L" RSHIFT " : L"        ";
			state_.hStateText_ += state_.keyboardMouseFlags_ & KeyboardMouseFlagBits::eKeyLeftCtrl ? L" LCTRL " : L"       ";
			state_.hStateText_ += state_.keyboardMouseFlags_ & KeyboardMouseFlagBits::eKeyRightCtrl ? L" RCTRL " : L"       ";
			state_.hStateText_ += state_.keyboardMouseFlags_ & KeyboardMouseFlagBits::eKeyLeftAlt ? L" LALT " : L"      ";
			state_.hStateText_ += state_.keyboardMouseFlags_ & KeyboardMouseFlagBits::eKeyRightAlt ? L" RALT " : L"      ";

			state_.hStateText_ += state_.keyboardMouseFlags_ & KeyboardMouseFlagBits::eMouseLeft ? L" LMB " : L"     ";
			state_.hStateText_ += state_.keyboardMouseFlags_ & KeyboardMouseFlagBits::eMouseMiddle ? L" MW " : L"    ";
			state_.hStateText_ += state_.keyboardMouseFlags_ & KeyboardMouseFlagBits::eMouseRight ? L" RMB " : L"     ";
			state_.hStateText_ += std::to_wstring(state_.mouseWheelDelta_);
			...
			state_.tNow_ = std::chrono::steady_clock::now();
			state_.tElapsedFPS_ = state_.tNow_ - state_.tStartFPS_;

			if (std::chrono::duration_cast<std::chrono::milliseconds>(state_.tElapsedFPS_).count() >= 1'000)
			{
				state_.tStartFPS_ = std::chrono::steady_clock::now();
				state_.FPS_ = state_.frameIdx_;
				state_.frameIdx_ = 0;
			}
			else
			{
				state_.tNow_ = std::chrono::steady_clock::now();
				state_.tElapsed_ = state_.tNow_ - state_.tStart_;

				std::this_thread::sleep_for(std::chrono::microseconds(state_.drawFrameMicrosecondsDuration_ -
					std::chrono::duration_cast<std::chrono::microseconds>(state_.tElapsed_).count()));
			}
		}
	}
	...
	LRESULT CALLBACK ak::Application::parentWndProc_(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_SYSKEYDOWN:
		case WM_SYSKEYUP:
			state_.keyboardMouseFlags_ = (state_.keyboardMouseFlags_ & (~KeyboardMouseFlagBits::eKeyLeftShift)) |
				(KeyboardMouseFlagBits::eKeyLeftShift & ((GetKeyState(VK_LSHIFT) & 0x8000) ? 0xFFFF : 0));
			state_.keyboardMouseFlags_ = (state_.keyboardMouseFlags_ & (~KeyboardMouseFlagBits::eKeyRightShift)) |
				(KeyboardMouseFlagBits::eKeyRightShift & ((GetKeyState(VK_RSHIFT) & 0x8000) ? 0xFFFF : 0));
			state_.keyboardMouseFlags_ = (state_.keyboardMouseFlags_ & (~KeyboardMouseFlagBits::eKeyLeftCtrl)) |
				(KeyboardMouseFlagBits::eKeyLeftCtrl & ((GetKeyState(VK_LCONTROL) & 0x8000) ? 0xFFFF : 0));
			state_.keyboardMouseFlags_ = (state_.keyboardMouseFlags_ & (~KeyboardMouseFlagBits::eKeyRightCtrl)) |
				(KeyboardMouseFlagBits::eKeyRightCtrl & ((GetKeyState(VK_RCONTROL) & 0x8000) ? 0xFFFF : 0));
			state_.keyboardMouseFlags_ = (state_.keyboardMouseFlags_ & (~KeyboardMouseFlagBits::eKeyLeftAlt)) |
				(KeyboardMouseFlagBits::eKeyLeftAlt & ((GetKeyState(VK_LMENU) & 0x8000) ? 0xFFFF : 0));
			state_.keyboardMouseFlags_ = (state_.keyboardMouseFlags_ & (~KeyboardMouseFlagBits::eKeyRightAlt)) |
				(KeyboardMouseFlagBits::eKeyRightAlt & ((GetKeyState(VK_RMENU) & 0x8000) ? 0xFFFF : 0));

			upVulkanSystem_->updateKeyboardMouseFlags(state_.keyboardMouseFlags_);
			break;
		case WM_DESTROY:
#ifdef _DEBUG
	...
	LRESULT CALLBACK ak::Application::workspaceWndProc_(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_SETCURSOR:
			GetCursorPos(&state_.screenCursorPoint_);
			state_.cursorPosition_ = INT32(DefWindowProc(hWnd, WM_NCHITTEST, NULL,
				MAKELPARAM(state_.screenCursorPoint_.x, state_.screenCursorPoint_.y)));
			...
			upVulkanSystem_->updateMouseCursorPosition(state_.workspaceCenteredCursorPoint_);
			upVulkanSystem_->draw();
			break;
		case WM_LBUTTONDOWN:
			state_.keyboardMouseFlags_ = state_.keyboardMouseFlags_ | KeyboardMouseFlagBits::eMouseLeft;
			upVulkanSystem_->updateKeyboardMouseFlags(state_.keyboardMouseFlags_);
			break;
		case WM_LBUTTONUP:
			state_.keyboardMouseFlags_ = state_.keyboardMouseFlags_ & (~KeyboardMouseFlagBits::eMouseLeft);
			...
			break;
		case WM_MBUTTONDOWN:
			state_.keyboardMouseFlags_ = state_.keyboardMouseFlags_ | KeyboardMouseFlagBits::eMouseMiddle;
			upVulkanSystem_->updateKeyboardMouseFlags(state_.keyboardMouseFlags_);
			break;
		case WM_MBUTTONUP:
			state_.keyboardMouseFlags_ = state_.keyboardMouseFlags_ & (~KeyboardMouseFlagBits::eMouseMiddle);
			...
			break;
		case WM_RBUTTONDOWN:
			state_.keyboardMouseFlags_ = state_.keyboardMouseFlags_ | KeyboardMouseFlagBits::eMouseRight;
			upVulkanSystem_->updateKeyboardMouseFlags(state_.keyboardMouseFlags_);
			break;
		case WM_RBUTTONUP:
			state_.keyboardMouseFlags_ = state_.keyboardMouseFlags_ & (~KeyboardMouseFlagBits::eMouseRight);
			...
			break;
		case WM_MOUSEWHEEL:
			state_.mouseWheelDelta_ = GET_WHEEL_DELTA_WPARAM(wParam) / WHEEL_DELTA;
			upVulkanSystem_->updateMouseWheelDelta(state_.mouseWheelDelta_);
			break;
		case WM_PAINT:
			upVulkanSystem_->draw();
			break;
