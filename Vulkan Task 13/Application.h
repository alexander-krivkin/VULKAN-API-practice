...
namespace ak
{
	...
	private:
		...
		void initWin32_();
#ifdef _DEBUG
		void initLoggingSystem_(const HWND& hWnd, uint32_t logLevel, std::string logFilename = "");
#endif // _DEBUG
		...
		struct
		{
			...
			LONG toolbarHeight_{ 50 }, stateHeight_{ 30 }, commandEditHeight_{ 100 };
			...
			uint32_t logLevel = LogLevelFlagBits::eAppWarning | LogLevelFlagBits::eAppError | LogLevelFlagBits::eVulkanWarning | LogLevelFlagBits::eVulkanError;
#endif // _DEBUG

			uint32_t targetFPS_{ 60 };
			uint32_t frameIdx_{}, FPS_{};
			long long drawFrameMicrosecondsDuration_{ 1'000'000 / targetFPS_ };
