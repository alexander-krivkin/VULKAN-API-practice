...
namespace ak
{
	...
	void Application::mainLoop_()
	{
		...
		while (true)
		{
			...
			hStateText_ = L"FPS: " + std::to_wstring(FPS) +
				L"  | Frame: " + 
				std::to_wstring(workspaceRect_.right - workspaceRect_.left) +
				L" ; " + std::to_wstring(workspaceRect_.bottom - workspaceRect_.top) +
				L"  | Cursor: " +
				std::to_wstring(screenCursorPoint_.x - screenWorkspaceCenterPoint_.x) +
				L" ; " + std::to_wstring(screenCursorPoint_.y - screenWorkspaceCenterPoint_.y);
	...
	LRESULT CALLBACK ak::Application::debugWndProc_(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_SETCURSOR:
		{
			if ((reinterpret_cast<HWND>(wParam) != hDebug_) && (!cursorVisible_))
			{
				ShowCursor(TRUE);
				cursorVisible_ = true;
			}
