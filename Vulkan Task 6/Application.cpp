...
namespace ak
{
	int Application::run()
	{
		...
		catch (const wexception& ex)
		{
			...
			ShowCursor(TRUE);
			MessageBox(hParent_, ex.what(), L"Исключение", MB_OK | MB_SYSTEMMODAL);
			return ex.code();
		}
		return EXIT_SUCCESS;
	}

	void Application::postMessage(LogLevel logLevel, const std::wstring& message)
	{
#ifdef _DEBUG
	...
	void Application::initWin32()
	{
		...
		{
			throw wexception(L"Невозможно зарегистрировать класс окна приложения");
		}
		...
		{
			throw wexception(L"Невозможно создать окно приложения");
		}
		...
		{
			throw wexception(L"Невозможно создать панель инструментов");
		}
		...
		if (!(hState_ = CreateWindowEx(
			...
			hParent_, reinterpret_cast<HMENU>(ElementID::eCtrlEditId), nullptr, nullptr)))
		{
			throw wexception(L"Невозможно создать текстовое поле");
		}
		...
		{
			throw wexception(L"Невозможно зарегистрировать класс окна рабочего пространства");
		}
		...
		{
			throw wexception(L"Невозможно создать окно рабочего пространства");
		}
		...
		{
			throw wexception(L"Невозможно зарегистрировать класс окна отладки");
		}
		...
		{
			throw wexception(L"Невозможно создать окно отладки");
		}
		...
		if (!(hDebugEdit_ = CreateWindowEx(
			...
			hDebug_, reinterpret_cast<HMENU>(ElementID::eCtrlDebugEditId), nullptr, nullptr)))
		{
			throw wexception(L"Невозможно создать текстовое поле окна отладки");
		}
...
#ifdef _DEBUG
	void Application::initLoggingSystem(const HWND& hWnd)
	{
	...
	void Application::mainLoop()
	{
		...
		while (true)
		{
			...
			if (std::chrono::duration_cast<std::chrono::milliseconds>(tElapsedFPS_).count() >= 1000)
			{
	...
	LRESULT CALLBACK ak::Application::parentWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_SIZE:
		{
			...
			catch (Exception& expt)
			{
				...
				MessageBox(hParent_, errorMessengeStream.str().c_str(), L"Исключение Vulkan", MB_OK | MB_SYSTEMMODAL);
